...
class gwogoaha_025:
    ...
    def move_swarm(self):
        for iter in range(self.iteration):
            ...
            if iter < self.iteration * 0.25 :
                a = self.Max - iter * self.Max /self.iteration
            else:
                a = self.Cmax - iter * (self.Cmax - self.Cmin) / self.iteration  

            for i in range(self.n_point):
                A1 = np.random.random(self.dimension) * ((2 * a * random.random()) - a)
                C1 = np.random.random(self.dimension) * (2 * random.random())
                A2 = np.random.random(self.dimension) * ((2 * a * random.random()) - a)
                C2 = np.random.random(self.dimension) * (2 * random.random())
                A3 = np.random.random(self.dimension) * ((2 * a * random.random()) - a)
                C3 = np.random.random(self.dimension) * (2 * random.random())
